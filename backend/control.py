...
async def restart_server(driver: uc.Chrome):
    try:
        restart_button = driver.find_element(By.XPATH, '//*[@id="restart"]')
    except:
        logger.warning('Server restart button was not found! Maybe, there is an error')
    else:
